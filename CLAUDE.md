...
- Email catalog SHIPPED (SPEC_EMAILS — source of truth for copy/triggers): all 8 emails in `src/server/email.ts`. Transactional: magic-link, welcome, subscription, mentor-booked (fires on `booked` flip in user.ts PATCH). Lifecycle cron `api/cron.ts` (CRON_SECRET-gated, Vercel Cron `0 9 * * *`): Thu weekly-tasks (if open tasks), Sat reflection, session-due→book-mentor, 14d-inactive→re-engagement. Dedup via `email_log` (`src/server/emailLog.ts` `sendOnce`); `users.lastActiveAt` activity signal (progress GET). Cron test hooks: `?day=N`/`?dry=1`/`?only=<email>`. All email/auth env vars now set in Vercel (incl. CRON_SECRET). Broadcasts (§5) = Resend dashboard, no code.
